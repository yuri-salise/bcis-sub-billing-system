      ...
      >
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflowY: 'auto' }}>
          <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div
              style={{
                backgroundColor: 'rgba(44, 87, 69, 0.08)',
                border: '1px solid rgba(44, 87, 69, 0.24)',
                borderRadius: '9px',
                padding: '14px 18px',
                display: 'flex',
                justifyContent: 'space-between',
            ...
            >
              <div>
                <span style={{ fontSize: '12px', color: '#2C5745', fontWeight: 600 }}>Total Current Due:</span>
                <div style={{ fontSize: '22px', fontWeight: 800, color: '#2C5745', letterSpacing: '-0.02em' }} className="tabular-nums">
                  {formatCurrency(totalBalanceCentavos ?? 0)}
                </div>
              </div>
            ...
            {hasDraftInvoices && (
              ...
              >
                <IconAlertTriangle size={16} strokeWidth={2} style={{ flexShrink: 0, color: 'var(--amber)' }} />
                <span>
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px' }}>
                {[
                  { id: PaymentMethod.CASH, label: 'Cash', icon: <IconBanknote size={16} strokeWidth={2} /> },
                  { id: PaymentMethod.GCASH, label: 'GCash', icon: <IconSmartphone size={16} strokeWidth={2} /> },
                  { id: PaymentMethod.BANK_TRANSFER, label: 'Bank Transfer', icon: <IconBuilding size={16} strokeWidth={2} /> },
                  { id: PaymentMethod.CHECK, label: 'Check', icon: <IconReceipt size={16} strokeWidth={2} /> },
                ].map((m) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        padding: '11px 8px',
                        borderRadius: '8px',
                        border: isSelected ? '2px solid #2C5745' : '1px solid var(--border-subtle)',
                        backgroundColor: isSelected ? 'rgba(44, 87, 69, 0.09)' : '#FFFFFF',
                        color: isSelected ? '#2C5745' : 'var(--text-secondary)',
                        ...
                        justifyContent: 'center',
                        gap: '6px',
                        boxShadow: isSelected ? '0 1px 4px rgba(44, 87, 69, 0.15)' : 'none',
                      }}
                    >
                      {m.icon}
                      <span>{m.label}</span>
            ...
            {tenderMethod === PaymentMethod.CASH && (
              <div
                style={{
                  backgroundColor: changeCentavos >= 0 ? 'var(--success-bg)' : 'var(--danger-bg)',
                  border: changeCentavos >= 0 ? '1px solid var(--success-border)' : '1px solid var(--danger-border)',
                  borderRadius: '8px',
                  padding: '12px 16px',
                  display: 'flex',
                  justifyContent: 'space-between',
              ...
              >
                <div>
                  <span style={{ fontSize: '12px', color: changeCentavos >= 0 ? 'var(--pine)' : '#991B1B', fontWeight: 600 }}>
                ...
                <div
                  ...
                  style={{
                    fontSize: '20px',
                    fontWeight: 800,
                    color: changeCentavos >= 0 ? 'var(--pine)' : '#DC2626',
                  }}
                >
                  {formatCurrency(Math.abs(changeCentavos ?? 0))}
                </div>
              </div>
            )}
            ...
            {allocationPlan && (
              <div style={{ border: '1px solid var(--border-subtle)', borderRadius: '8px', padding: '12px', backgroundColor: 'var(--bg-subtle)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {allocationPlan.allocations.map((a) => (
                    ...
                    >
                      <div>
                        <span className="font-mono" style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{a.invoiceNumber}</span>
                        <span style={{ color: 'var(--text-muted)', marginLeft: '8px' }}>
                          Prev: {formatCurrency(a.previousBalanceCentavos ?? 0)}
                        </span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ color: '#2C5745', fontWeight: 700 }} className="tabular-nums">
                          +{formatCurrency(a.allocatedAmountCentavos ?? 0)}
                        </span>
                        <span
                          className={`badge ${
                  ...
                  {allocationPlan.advanceCreditCentavos > 0 && (
                    ...
                    >
                      <span style={{ fontWeight: 600 }}>Excess Stored as Advance Credit:</span>
                      <span className="tabular-nums" style={{ fontWeight: 800 }}>
                        +{formatCurrency(allocationPlan.advanceCreditCentavos ?? 0)}
                      </span>
                    </div>
                  )}
                </div>
            ...
            {errorMessage && (
              <div style={{ fontSize: '12px', color: '#DC2626', padding: '10px 12px', backgroundColor: '#FEF2F2', border: '1px solid #FECACA', borderRadius: '6px', fontWeight: 500 }}>
                {errorMessage}
              </div>
            )}
          </div>
          ...
          >
            ...
            >
              {isSubmitting ? 'Posting Payment...' : `Post ${formatCurrency(paymentAmountCentavos ?? 0)}`}
            </button>
          </div>
        </form>
      </div>
